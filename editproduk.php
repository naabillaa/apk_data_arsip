...
$kodepelanggan = $namaperusahaan = $klasifikasi = $alamatperusahaan = $alamatpengiriman = $kontakperson = $npwp = $sales = $catatan = $tanggalupdate = "";
$proposal = "";
...
if (isset($_POST['submit'])) {
    ...
    // Periksa apakah file proposal baru diunggah
    if ($_FILES["proposal"]["name"]) {
        // Jika file baru diunggah, gunakan file yang baru
        $proposal_name = $_FILES["proposal"]["name"];
        $proposal_temp = $_FILES["proposal"]["tmp_name"];
        ...
        $target_file = $target_dir . basename($proposal_name);

        // Periksa apakah file memiliki ekstensi yang diizinkan
        $allowed_extensions = array("pdf", "doc", "docx");
        $file_extension = strtolower(pathinfo($target_file, PATHINFO_EXTENSION));
        if (!in_array($file_extension, $allowed_extensions)) {
            echo "ERROR: Hanya file PDF, DOC, dan DOCX yang diizinkan.";
            exit();
        }
        ...
        if (move_uploaded_file($proposal_temp, $target_file)) {
            // Jika berhasil diunggah, simpan nama file ke variabel proposal
        ...
        $proposal = $_POST["existing_proposal"];
    }

    // Query untuk melakukan update data
    $query_update = "UPDATE pelanggan SET kodepelanggan='$kodepelanggan', namaperusahaan='$namaperusahaan', klasifikasi='$klasifikasi', alamatperusahaan='$alamatperusahaan', alamatpengiriman='$alamatpengiriman', kontakperson='$kontakperson', npwp='$npwp', proposal='$proposal', sales='$sales', catatan='$catatan', tanggalupdate='$tanggalupdate' WHERE kodepelanggan='$kodepelanggan'";
...
// Mendapatkan nilai kodepelanggan dari URL
$kodepelanggan = $_GET['kodepelanggan'];

// Query untuk mendapatkan data pelanggan berdasarkan kodepelanggan
...
// Periksa hasil query
if ($result === false) {
    echo "ERROR: Gagal mengeksekusi query: " . mysqli_error($conn);
    exit();
}

// Pastikan ada baris yang dikembalikan
      ...
      <div class="col-md-8 offset-md-2">
              ...
              <div class="card-body">
                  ...
                  <div class="form-group">
                          ...
                          <input type="text" name="kodepelanggan" value="<?php echo $row['kodepelanggan'] ?>" placeholder=" Nama Produk " class="form-control"readonly>
                      </div>
                      
                      <div class="form-group">
                          <label>Nama Perusahaan</label>
                          <input type="text" name="namaperusahaan" value="<?php echo $row['namaperusahaan'] ?>" placeholder=" Nama Produk " class="form-control">
                      </div>

                      <div class="form-group">
                          <label>Klasifikasi</label>
                          <input type="text" name="klasifikasi" value="<?php echo $row['klasifikasi'] ?>" placeholder="harga" class="form-control">
                      </div>

                      <div class="form-group">
                          <label>Alamat Perusahaan</label>
                          <input type="text" name="alamatperusahaan" value="<?php echo $row['alamatperusahaan'] ?>" placeholder="harga" class="form-control">
                      </div>

                      <div class="form-group">
                          <label>Alamat Pengiriman</label>
                          <input type="text" name="alamatpengiriman" value="<?php echo $row['alamatpengiriman'] ?>" placeholder="Stok" class="form-control">
                      </div>

                      <div class="form-group">
                          <label>Kontak Person</label>
                          <input type="number" name="kontakperson" value="<?php echo $row['kontakperson'] ?>" placeholder="Stok" class="form-control">
                      </div>
                      
                     
                      <div class="form-group">
                          <label>NPWP</label>
                          <input type="text" name="npwp" value="<?php echo $row['npwp'] ?>" placeholder="Stok" class="form-control">
                      </div>

                      <div class="form-group">
                      <label>Proposal</label>
                      <!-- Tambahkan input file untuk unggah proposal -->
                      <input type="file" name="proposal" class="form-control">
                      <!-- Tambahkan input tersembunyi untuk menyimpan nama file proposal yang ada di database -->
                      <input type="hidden" name="existing_proposal" value="<?php echo $row['proposal']; ?>">
                      <?php if (!empty($row['proposal'])): ?>
                          <!-- Tampilkan nama file proposal yang tersimpan -->
                          <p>Nama file proposal yang tersimpan: <?php echo $row['proposal']; ?></p>
                      <?php endif; ?>
                  </div>
                  ...
                  <div class="form-group">
                          <label>Sales</label>
                          <input type="text" name="sales" value="<?php echo $row['sales'] ?>" placeholder="Stok" class="form-control">
                      </div>

                      <div class="form-group">
                          <label>Catatan</label>
                          <input type="text" name="catatan" value="<?php echo $row['catatan'] ?>" placeholder="Stok" class="form-control">
                      </div>
...
&copy; 2024 <a href="https://www.creative-tim.com/" class="font-weight-bold ml-1" target="_blank">Nbilla Arifia</a>
</div>
</div>
<div class="col-lg-6">
