...
// Default value for kodepelanggan if not set in GET request
$kodepelanggan = isset($_GET['kodepelanggan']) ? $_GET['kodepelanggan'] : "Nilai Default";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Prepare SQL statement for inserting into produk table
    $sql_produk = "INSERT INTO produk (kodeproduk, namaproduk, satuan, tanggal, jumlah, harga, tanggal2, jumlah2, harga2, spesifikasiproduk, fotoproduk, fotoproduk2, kodepelanggan) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt_produk = $conn->prepare($sql_produk);

    if ($stmt_produk) {
        // Bind parameters for produk table
        $stmt_produk->bind_param("sssssssssssss", $kodeproduk, $namaproduk, $satuan, $tanggal, $jumlah, $harga, $tanggal2, $jumlah2, $harga2, $spesifikasiproduk, $fotoproduk, $fotoproduk2, $kodepelanggan);
        ...
        // Execute the produk table insert
        if ($stmt_produk->execute()) {
            $produk_id = $stmt_produk->insert_id;

            // Prepare SQL statement for inserting into kelompok table
            $sql_kelompok = "INSERT INTO kelompok (kelompokproduk, jumlah, tanggalcek, keterangan, kodeproduk, kodepelanggan) VALUES (?, ?, ?, ?, ?, ?)";
            $stmt_kelompok = $conn->prepare($sql_kelompok);

            if ($stmt_kelompok) {
                // Check if arrays are set and are arrays
                if (isset($_POST['kelompokproduk']) && is_array($_POST['kelompokproduk']) &&
                    ...
                    for ($i = 0; $i < $count; $i++) {
                        ...
                        $stmt_kelompok->bind_param("ssssss", $kelompokproduk, $jumlahproduk, $tanggalcek, $keterangan, $kodeproduk, $kodepelanggan);
...
?>
    ...
    <li class="breadcrumb-item active" aria-current="page">Tambah Produk Pelanggan</li>
    </ol>
    </nav>
    </div>
    ...
    <h3 class="mb-0">Tambah Produk Pelanggan 
    ...
    <!-- <br> -->
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="spesifikasiproduk" placeholder="Spesifikasi Produk " class="form-control"required>
                </div>
                <div class="form-group">
